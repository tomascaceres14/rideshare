...
// Consumes messages from a Queue and resends the message to the WebSocket connection via ConnectionManager
...
// Redirects via websocket connection the received queue message
func (qc *QueueConsumer) Start() error {
	msgs, err := qc.rb.Chan.Consume(
		qc.queueName,
		"",
		true,
		false,
